-- DROP FUNCTION config.f_get_tick_tables_cleanup_query(_interval_unit TEXT, _interval_value TEXT);

CREATE OR REPLACE FUNCTION config.f_get_tick_tables_cleanup_query(_interval_unit TEXT, _interval_value TEXT)
    RETURNS text
    LANGUAGE plpgsql
AS
    $$
    DECLARE
        ...
        _drop_data_stg_view_query  TEXT := 'DROP VIEW stg.data' || _interval_name || ';';
